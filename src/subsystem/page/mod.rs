...
impl ArmPageDescriptor {
    ...
    pub const fn get_sh(&self) -> u64 {
        (self.0 & SH_MASK) >> SH_OFFSET
    }

    pub const fn set_sh(&mut self, sh: u64) {
        self.0 &= !SH_MASK;
        self.0 |= (sh << SH_OFFSET) & SH_MASK;
    }
...
impl ArmDescriptorGroupManager {
    ...
    pub fn split_block_descriptor(&self, index: usize) {
            ...
            unsafe { new_group_descriptors.as_mut().0.iter_mut().enumerate() }
        {
            ...
            new_descriptor.set_sh(descriptor.get_sh());
